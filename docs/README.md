...
| `canonical-ast.md` | AST shape, canonicalisation rules, per-unit identity (signature vs body hashing), and the determinism invariants. | not written — needed for M3. The AST is already *shaped* for it (`lex-sys-syntax`), but nothing hashes it yet |
