...
| `strings.md` | What a string is: bytes rather than an encoding, `byte` as storage rather than arithmetic, packed `[byte]` layout, literals and the static region, and what crosses to C. | **written, not built** — the gate for M3's last item, with its must-reject suite stated in advance (§9) |
