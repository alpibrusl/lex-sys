...
| Next | Why it is next |
|---|---|
| Compile-time **data** | `compile-time.md` §8 — the one that pays. `examples/base64/` scans 64 entries per decoded character where GNU uses a 256-entry table, and its own comment says why: a table is a lot of source for something a loop settles. Evaluating a pure function that returns a *slice* into static bytes is where C programmers reach for a generator script, which is the definition of a place the compiler gave up |
