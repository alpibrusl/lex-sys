...
| The rest of the string library | `split`, `trim`, `join`, ordering, case. [`utf8.md`](utf8.md) settled the one decision and `std.utf8` is built; what is left is code with nothing to decide, and the probe showed the language needs nothing for any of it. `std/bytes.ls` is 125 lines today and this is the cheapest genuinely useful slice on this list |
