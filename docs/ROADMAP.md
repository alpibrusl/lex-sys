...
| The string library | [`utf8.md`](utf8.md) settles the one decision; the rest — `split`, `trim`, `join`, ordering, case — is code with nothing left to decide, and the probe showed the language needs nothing for any of it. `std/bytes.ls` is 125 lines today and this is the cheapest genuinely useful slice on this list. The decoder ships with §2's seven fixtures, since agreeing with *somebody* is the only reason to count code points |
